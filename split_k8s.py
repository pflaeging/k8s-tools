...
def process_document(doc, output_dir, yaml_parser, unwrap_lists=True, verbose=False):
    ...
    if unwrap_lists and kind == "List" and "items" in doc:
        ...
        for item in doc["items"]:
            list_count += save_resource(item, output_dir, yaml_parser, verbose)
        return list_count
    else:
        # Standard processing for single objects (or Lists if unwrapping is disabled)
        return save_resource(doc, output_dir, yaml_parser, verbose)


def save_resource(doc, output_dir, yaml_parser, verbose=False):
...
def split_kubernetes_yaml(yaml_stream, output_dir, unwrap_lists=True, verbose=False):
    ...
    try:
        ...
        for doc in documents:
            total_saved += process_document(
                doc, output_dir, yaml_parser, unwrap_lists, verbose
            )
...
def main():
    ...
    try:
        split_kubernetes_yaml(
            args.input, args.output_dir, args.unwrap_lists, args.verbose
        )
    finally:
